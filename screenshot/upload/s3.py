...
class S3(UploadPlugin):
   def upload(self, meta, localfile, shortname):
      conn = S3Connection(
         aws_access_key_id=self.config.s3_config['key'],
         aws_secret_access_key=self.config.s3_config['secret'],
         host=self.config.s3_config['endpoint']
      )
      bucket = conn.get_bucket(self.config.s3_config['bucket'])
      ...
      s3_url = "http://%s.%s/%s" % (
         ...
         self.config.s3_config['endpoint'],
         s3_key
      )
